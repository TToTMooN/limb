...
def main(args: Args) -> None:
    ...
    # Build blueprint — explicit layout with one TimeSeriesView per (arm, signal)
    views = []
    for arm_name in arm_names:
    ...
    if not args.no_video:
        for cam_name in cam_names:
            ...
            # Convert ms → s if timestamps look like milliseconds (>1e12)
    ...
    # Keep process alive so data stays in the viewer
    try:
        import signal
