...
class DAggerAgent(Agent):
    ...
    def __post_init__(self) -> None:
        ...
        # can stiff-track the followers.  Subsequent mode switches are queued
        # by phase transitions in `_apply_transition`.
        self._leader_mode_to_emit: Optional[str] = "position"
    ...
    def act(self, obs: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        if self._leader_mode_to_emit is not None:
            for lkey in self.leader_keys:
                entry = action.setdefault(lkey, {})
                entry["mode"] = self._leader_mode_to_emit
            self._leader_mode_to_emit = None
    ...
    def _apply_transition(self, old_phase: DAggerPhase, new_phase: DAggerPhase) -> None:
        ...
        if new_phase is DAggerPhase.CORRECTING:
            ...
            # whatever inner_teleop returns this tick.
            self._leader_mode_to_emit = "zero_torque"
