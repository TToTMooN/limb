    ...
    def step(
        ...
        *,
        ...
        Args:
            ...
            intervention: ``True`` if this frame was generated by an operator
                correction (DAgger CORRECTING phase).  Forwarded to
                ``recorder.record(...)`` and saved per-tick.  Defaults to
                ``False`` for backwards compatibility with non-DAgger agents.
            phase: Current DAgger phase string (ignored here; accepted so
                ``launch.py`` can plumb a single signature to all sessions).
        """
        del phase  # unused; accepted for signature compatibility
        if self._done:
            return False
